...
class MLClient:
    ...
    def _build_prompt(self, payload: Dict[str, Any], candidates: List[Dict[str, Any]]) -> str:
        ...
        header = (
            ...
            "User constraints:\n"
            f"- Must include ingredients (preferred): {includes}\n"
            f"- Must exclude ingredients (allergy/dislike): {excludes}\n"
            f"- User DOES NOT HAVE these ingredients: {dont_have}\n"
            f"- User HAS these tools/equipment: {tools_have}\n"
            f"- Preferred cuisine: {cuisine}\n"
            f"- Taste profile: {taste}\n"
            f"- Dietary restrictions: {diet}\n\n"
            "When choosing among candidates:\n"
            "- Strongly prefer recipes that do NOT require ingredients the user does not have.\n"
            "- Prefer recipes whose tools are a subset of the user's available tools.\n"
            "- Only choose a candidate that violates these if no better option exists, and then\n"
            "  clearly explain substitutions or alternative methods using ONLY the user's tools.\n\n"
            "Candidate recipes:\n"
        )
